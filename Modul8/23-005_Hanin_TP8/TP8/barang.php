...
if (isset($_POST['tambah'])) {
    ...
    if (tambahBarang($kode, $nama, $harga, $stok, $id_supplier)) {
...
if (isset($_POST['update'])) {
    ...
    if (updateBarang($id, $kode, $nama, $harga, $stok, $id_supplier)) {
        $success_message = "Data barang berhasil diupdate!";
    } else {
        $error_message = "Gagal mengupdate data barang!";
    }
}
...
if (isset($_GET['hapus'])) {
    ...
    if (hapusBarang($id)) {
        ...
        $error_message = "Gagal menghapus data barang!";
    }
}
...
if (isset($_GET['edit'])) {
    $editMode = true;
    $id_edit = $_GET['edit'];
    $dataEdit = getBarangById($id_edit);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <div class="container mt-4">
        <h4 class="h4 bg-primary text-white py-3 ps-4 mb-0">Data Master Barang</h4>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <h5 class="card-title"><?= $editMode ? 'Edit Barang' : 'Tambah Barang' ?></h5>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="kode_barang" id="kode_barang" 
                                   value="<?= $editMode ? $dataEdit['kode_barang'] : '' ?>" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="nama_barang" class="form-label">Nama Barang</label>
                            <input type="text" class="form-control" name="nama_barang" id="nama_barang" 
                                   value="<?= $editMode ? $dataEdit['nama_barang'] : '' ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" name="harga" id="harga" 
                                   value="<?= $editMode ? $dataEdit['harga'] : '' ?>" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="stok" class="form-label">Stok</label>
                            <input type="number" class="form-control" name="stok" id="stok" 
                                   value="<?= $editMode ? $dataEdit['stok'] : '' ?>" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select" name="id_supplier" id="id_supplier" required>
                                ...
                                <option value="<?= $supplier['id'] ?>" 
                                    <?= ($editMode && $dataEdit['id_supplier'] == $supplier['id']) ? 'selected' : '' ?>>
                    ...
                    <table class="table table-hover mb-0 table-bordered">
                        ...
                        <tbody>
                            <?php if (count($dataBarang) > 0): ?>
                                <?php foreach ($dataBarang as $nomor => $barang): ?>
                                <tr>
                                    <td><?= $nomor + 1 ?></td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="?edit=<?= $barang['id'] ?>" class="btn btn-sm btn-warning">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>
                                            <a href="?hapus=<?= $barang['id'] ?>" class="btn btn-sm btn-danger" 
